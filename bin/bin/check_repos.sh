...
show_help() {
    cat << 'EOF'
check_repos.sh — Report uncommitted changes in all git repos under ~

Output format: repo_path: 2M/1A/3?
...
Examples:
  /home/user/project: 2M/1A/3?   (2 modified, 1 added, 3 untracked)
  /home/user/website: 1M/2D      (1 modified, 2 deleted)

Usage:
  check_repos.sh              Show all repos with changes (one-line summary)
...
# Find all git repos and check for uncommitted changes

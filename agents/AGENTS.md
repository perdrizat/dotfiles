...
- **Put scratch files in the repo's `.tmp/`, never `/tmp`.** Always use a gitignored `.tmp/` directory at the repo root as your scratch space, and create it (`mkdir -p .tmp`) if it's missing. Writes *inside* the workspace don't prompt and reads never do, so `.tmp/` stays entirely within auto-approved behaviour. Temp files under `/tmp` prompt instead: writes land outside the workspace, and building them with a shell `cat`/redirect trips reasons 1–2 anyway. Don't blanket-allowlist `/tmp`: it's world-writable and shared, so a broad allow lets you touch other processes' temp data, and it wouldn't even silence the redirect/`cat` prompts that cause most of the friction.

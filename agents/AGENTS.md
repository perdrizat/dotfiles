...
- **Put scratch files where you're already trusted to write — not `/tmp`.** Temp files under `/tmp` prompt: writes land outside the workspace, and building them with a shell `cat`/redirect trips reasons 1–2 anyway. Instead use the Read/Write tools against a location that's already approved — the agent's own session scratch directory if it provides one, otherwise a gitignored `.tmp/` inside the repo (writes *inside* the workspace don't prompt, and reads never do). Don't blanket-allowlist `/tmp`: it's world-writable and shared, so a broad allow lets you touch other processes' temp data, and it wouldn't even silence the redirect/`cat` prompts that cause most of the friction.
